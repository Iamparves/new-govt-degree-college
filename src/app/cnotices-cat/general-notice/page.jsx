import { generalNoticeList } from "@/data/noticeList";
...
const page = () => {
  return (
    <section>
      <h2 className="mb-3 text-3xl font-bold">General Notice</h2>
      <div className="overflow-x-auto overflow-y-hidden">
        <table className="w-full border-collapse border border-[#ddd] text-left text-sm text-dark md:text-base">
          ...
          <tbody>
            {generalNoticeList.map((notice, index) => {
              return (
                <tr
                  key={index}
                ...
                >
                  <td className="border border-[#ddd] p-2.5">
                    <Link href={`/cnotices-cat/general-notice/${notice.id}`}>
                      {notice.title}
                    </Link>
                  </td>
